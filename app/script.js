...
function displayResults(data) {
    ...
    sentimentCountsDiv.innerHTML = '<h3>Sentiment Counts:</h3>' +
        data.sentiment_counts.map(count => `<p>${count.Sentiment}: ${count.Count}</p>`).join('');
    
    // Populate Sorted Comments
    const sortedCommentsDiv = document.getElementById('sortedComments');
    sortedCommentsDiv.innerHTML = '<h3>Sorted Comments:</h3>' +
        data.sorted_comments.map(comment => `<p>${comment.SerialNo}. ${comment.Label}: ${comment.Comment}</p>`).join('');
    
    // Populate Summaries
    const summariesDiv = document.getElementById('summaries');
    summariesDiv.innerHTML = '<h3>Summaries:</h3>' +
        Object.entries(data.summary_by_sentiment).map(([sentiment, summary]) => `<p>Summary for ${sentiment}: ${summary}</p>`).join('');
}
